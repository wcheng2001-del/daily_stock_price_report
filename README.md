...
报告会优先使用当天可取得的最新交易数据；若 Yahoo Finance 的日线资料刷新较慢，程序会以最新分钟线补齐当天的开盘、高低、最新价与成交量。

若股票的可取得历史不足一年，报告仍会列出可用的价格与图表；52 周高／低会标示为现有数据区间，尚无法可靠计算的长周期指标会显示 `—`。
